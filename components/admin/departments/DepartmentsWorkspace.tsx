        ...
        <div className={cn(surface("default"), "overflow-hidden")}>
          {filtered.length === 0 ? (
            ...
            <div className="divide-y divide-slate-100 md:divide-y-0">
              ...
              <div className="hidden bg-slate-50 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 md:grid md:grid-cols-[1.4fr_1fr_0.6fr_0.7fr_auto]">
                <div className="px-4 py-3">Department</div>
                <div className="px-4 py-3">Head</div>
                <div className="px-4 py-3">Teachers</div>
              ...
              {filtered.map((row) => (
                <div
                  key={row.id}
                  className="grid grid-cols-1 gap-2 p-4 transition hover:bg-slate-50/70 md:grid-cols-[1.4fr_1fr_0.6fr_0.7fr_auto] md:items-start md:gap-0 md:border-t md:border-slate-100 md:px-4 md:py-3"
                >
                  {/* Department */}
                  ...
                  {/* Head */}
                  <div className="min-w-0">
                    <span className="mb-0.5 block text-[10px] font-semibold uppercase text-slate-400 md:hidden">
                      Head
                    </span>
                    {quickAssignId === row.id ? (
                      ...
                      <div>
                        <p
                          className={cn(
                            "font-medium",
                            row.head ? "text-slate-800" : "text-amber-800",
                          )}
                        >
                  ...
                  <div className="flex items-center gap-2 md:justify-end">
                    <button
                      ...
                      className="flex min-h-[36px] flex-1 items-center justify-center gap-1.5 rounded-lg border border-slate-200 px-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 md:h-8 md:w-8 md:flex-none md:rounded-md md:px-0"
                      aria-label={`Assign head for ${row.name}`}
                      title="Assign head"
                    >
                      <UserPlus className="h-3.5 w-3.5" />
                      <span className="md:hidden">Head</span>
                    </button>
                    <button
                      type="button"
                    ...
                    {!row.is_default ? (
                      <button
                        ...
                        className="grid h-9 w-9 place-items-center rounded-lg border border-rose-200 text-rose-600 transition hover:bg-rose-50 md:h-8 md:w-8 md:rounded-md md:border-slate-200"
